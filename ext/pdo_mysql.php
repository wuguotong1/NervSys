...
class pdo_mysql extends pdo
{
    ...
    public function join(string $table,array $join,string $type = 'inner')
    {
        if(is_array($join) && !empty($join))
        {
            ...
            if($str != '')
            {
                $this->param_join .= ltrim($str,' AND');
            }
        }
        unset($table,$join,$type,$str,$k,$v);
        return $this;
    }
    ...
    public function fetchAll(array $field =['*'],int $fetch_style = \PDO::FETCH_ASSOC)
    {
        $this->param_field = implode(',',$field);
        if($this->param_field != '*')
            $this->select($this->param_table);
